...
- **table alignment** - `build/fix-tables.py` already re-pads columns and asserts cell contents are
  ...
  hand-pad tables.
- **bare acronyms** - tried and dropped in the companion `ml-engineering` book. Flagging any acronym lacking a gloss in the same file gave hundreds of hits, because ordinary vocabulary (`CUDA`, `GDB`, `NCCL`, `SLURM`) dominated. Telling "vocabulary the reader has" from "jargon needing a gloss" is a judgement about audience.
- **non-ASCII maths symbols** - tried and dropped for the same reason: too many intentional style hits relative to real defects.
