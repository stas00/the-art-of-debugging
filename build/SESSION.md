...
20. **American English in author prose.** New sentences use American spelling: `neighbor`, `behavior`, `analyze`, `color`, `organize`. Keep a different spelling when it is an identifier, an API, a filename, or a quoted vendor string. The same rule holds in the companion book.
...
3. Prefer saying a number is unknown over supplying a plausible one. "This has not been measured on that hardware" is worth more to a reader than any figure that would need to be labeled unreliable.
...
4. The tell is that it feels like good style. A section written in strong first person invites continuation in the same key, and the more convincingly the voice is copied the less likely anyone is to question it. Treat the urge to add color as the signal to stop.
...
1. In-book links use the file form `pytorch/README.md#anchor`, never the bare-directory form `pytorch#anchor`. The compiled HTML/PDF/epub build resolves the file form to a real anchor, but a bare directory has no anchor target in the built book, and `build/check-links.py` flags `dir#anchor` as `ANCHOR ON DIRECTORY`. A directory link with *no* fragment is fine and needs no change.
2. GitHub-web caveat for large files: GitHub's blob (file) view does not serve heading-anchor ids for large Markdown files, so an absolute deep link such as `.../blob/master/pytorch/README.md#anchor` does not scroll to the section on a cold load - the browser lands at the top. Measured 2026-08-13: a ~5 KB README embeds its anchors, the ~215 KB `pytorch/README.md` embeds none. Same-page `#anchor` clicks still work; only cold navigation into the big file breaks.
3. Remedy for that case only: for an *absolute* cross-repo deep link into a large folder-README, use the folder (`tree`) form `https://github.com/OWNER/REPO/tree/master/pytorch#anchor` instead of `.../blob/master/pytorch/README.md#anchor`. The tree view serves anchors at any size, and a directory URL 301-redirects `blob/master/pytorch` -> `tree/master/pytorch` with the browser carrying the fragment. Use this only for absolute/external links (`check-links.py` skips them and the compiled book treats them as external), and only when the section lives in that folder's `README.md` - a non-README file such as MLE's `debug/pytorch.md` has no folder form.
4. Do not use the folder form for relative in-book links, and do not teach `check-links.py` to resolve a directory to its `README.md`. GitHub's *web* view now redirects a directory to its tree README (the 2026-08-02 premise that "a fragment on a directory has nothing to bind to" changed by 2026-08-13), but the compiled book and local renderers still do not, so the file form stays mandatory in-book. `build/check-links.py` deliberately does not append `README.md`.
...
9. This book's one oversized deep-link target is `pytorch/README.md`, and it *is* a folder README - so the item 3 remedy applies: absolute/cross-repo links into it take the tree form `https://github.com/stas00/the-art-of-debugging/tree/master/pytorch#anchor` (converted 2026-08-13 in `SKILL.md` and in the companion book's inbound links). Relative in-book links into it keep the file form per items 1 and 4 and rely on reload / the file Outline panel for the item 2 cold-anchor case.
...
2. This covers every tracking file: `build/consistency-review-*.md`, `build/update-suggestions-*.md`, `build/stabs-actionable-*.md`, and `incoming.md`.
...
10. So when a chapter is edited because a dependency gained a feature, grep the neighboring files for older statements about that same dependency. The new measurement is what reveals the old claim - nothing else will, because a stale "you can't" reads exactly like a true one.
11. The neighbor can be the next paragraph. Sweeping a chapter for a claim's neighbors is not enough if you do not also verify that the claim was true when written.
12. **Environment variable names get deprecated and renamed upstream, so verify each one the book tells a reader to set is still the current name.** This is the dependency-rot shape (items 9-11) in its purest form: a deprecated env var keeps working as an alias and at most prints a warning, so nothing breaks, every functional test passes, and the stale name survives every review that only reads for correctness. Sweep the vars the book asks a reader to `export` or pass and confirm each against the tool's current documentation rather than trusting that it still runs. Concrete, 2026-08-13: `PYTORCH_CUDA_ALLOC_CONF` had become the device-agnostic `PYTORCH_ALLOC_CONF` and `pytorch/README.md` already taught the new name while `SKILL.md` still shipped the old one; the flight-recorder pair `TORCH_NCCL_TRACE_BUFFER_SIZE`/`TORCH_NCCL_DEBUG_INFO_TEMP_FILE` likewise became `TORCH_FR_BUFFER_SIZE`/`TORCH_FR_DUMP_TEMP_FILE` in PyTorch 2.9 (the companion book's launcher templates had the same problem with `NCCL_ASYNC_ERROR_HANDLING` → `TORCH_NCCL_ASYNC_ERROR_HANDLING`). Prefer the current name in the book; name a superseded one only to say it was renamed.
...
This book is not a hardware-spec catalog; keep this section short. When a rare comparison table does appear:
