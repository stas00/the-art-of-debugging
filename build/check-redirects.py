...
different domains proceed in parallel. Never remove this. Firing many concurrent requests at a
single host (GitHub, Hugging Face, docs sites) looks like a scraper and gets the runner throttled
or IP-blocked, which is far more expensive than a slow check. Budget hours for a full-book run,
not minutes. Raise --jobs to cover more domains at once, not to go faster on one.
...
# failure (companion book): a tools chapter said to visit `github.com/.../commit/` "and append
# the commit SHA", and that was reported as having moved to /commit/main - a "fix" that would
# have broken the instruction. Strip code-span URLs so template paths are not treated as citations.
# Note this deliberately does NOT skip ``` fences ```, unlike build/check-links.py: URLs that
# appear only inside fences can still be live download links whose death would break setup
# instructions, so those stay checked.
