...
The left side visualization is very telling to how gpu memory is used in the `forward` and `backward` calls, you can see how the left side is the layer-by-layer `forward` calls, and the right side is layer-by-layer `backward` calls. You can also see that `backward` takes 2x longer than `forward` because it has do to compute gradients wrt weights and inputs.

The right side visualization shows how very different memory usage pattern is if we don't store any intermediary tensors on the gpu and offload them to cpu memory. The graph is flat, so you can add many more layers and it'll still use the same amount of gpu memory, whereas the image on left shows that if there are too many layers one will run into OOM. If you're curious, the big spikes during `backward` in both images are gradient reductions across gpus.
