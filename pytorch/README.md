...
Unless one writes their own `torch.autograd` class, PyTorch will autogenerate the `backward` function corresponding to each `forward` class it runs. And since it is autogenerated, whenever there is an issue in `backward` it becomes very difficult to find what `forward` call is responsible for it and thus understand the cause of the problem.

First, it's quite tricky to write a simple script to create a CUDA OOM in an auto-generated `backward`, because when `backward` starts, PyTorch immediately drops the temporary tensors from `forward`, freeing memory for `backward` memory allocations to almost always be successful.
...
And let's lower the `x` size to 10B elements so that `forward` doesn't OOM, but consumes most of the memory leading to a CUDA OOM in `backward`.
...
As you can see this traceback tells us absolutely nothing about where the problem is and thus in a real world where there could be dozens of classes and layers it's quite impossible to identify the corresponding `forward` call other than perhaps via some smart bisection of the `forward` calls (shortcutting them a few a time), which would be quite difficult and time consuming and error-prone.

Luckily, PyTorch has a special [autograd mechanism that is used to detect anomalies](https://docs.pytorch.org/docs/stable/autograd.html#debugging-and-anomaly-detection). When enabled it'll augment the `backward` backtrace with the `forward` backtrace corresponding to the `backward` code. Let's move the implicit `forward` and explicit `backward` calls into the `torch.autograd.detect_anomaly()` context manager:
...
footnote: it's crucial that both the `forward` and the `backward` calls are inside the context manager.

When we run this updated script:
...
So now we know exactly where in the `forward` code there is an issue that gets triggered in the `backward`.

### Overcoming CUDA OOM due to memory fragmentation

When developing [Arctic Long Sequence Training](https://arxiv.org/abs/2506.13996) a lot of the work involved hard reshaping tensors (i.e. `contiguous` calls) for Sequence Parallelism, which lead to a very poor HBM utilization, because the reshapes lead to memory fragmentation.
...
In the particular case of this project I haven't observed any noticeable performance degradation, but if you use it do benchmark the performance w/ and w/o it to ensure it doesn't impact your workload for the worse.
